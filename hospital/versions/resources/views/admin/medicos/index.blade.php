    ...
    <div class="white-box">
      ...
      <div class="pull-left">
        <?php if(Auth::user()->permisos->addRecord == 1) { ?>
          <a href="{{ url('/admin/medicos/add') }}" class="btn btn-info ">
            <i class="fa fa-plus fa-2x"></i><br/>Nuevo
          </a>
        <?php } ?>

        <button type="button" class="btn btn-success" onclick="buscaPaciente()">
          <i class="fa fa-search fa-2x"></i><br/>Buscar
        </button>
      </div>

      <div class="pull-right">
        <a href="{{ url('/admin/medicos/excel' . $query) }}" class="btn btn-success" title="Exportar a Excel">
          <i class="fa fa-copy fa-2x"></i><br/>E. Excel
        </a>

        <a href="{{ url('/admin/medicos/pdf' . $query) }}" class="btn btn-danger" title="Exportar a PDF">
      				...
      				<div class="col-md-4">
      				 <div class="form-group">
      					<label for="especialidad" class="control-label"> Especialidad </label>
      						...
      						value="{{{ isset($data->especialidad ) ? $data->especialidad  : old('especialidad') }}}">
            ...
            <div class="white-box">
                <div class="row">
                    <div class="col-md-4 col-sm-4 text-center">
                      <a href="{{{ url('admin/medicos/view/' . $value->id) }}}">
                      <?php if($value->fotografia) { ?>
                        <img src="{{{ asset('uploads/medicos/' . $value->fotografia) }}}" alt="user" class="img-circle img-responsive">
                      <?php } else { ?>
                        <img src="{{{ asset('uploads/medico.png') }}}" alt="user" class="img-circle img-responsive">
                      <?php } ?>
                      </a>
                    ...
                    <div class="col-md-8 col-sm-8">
                        <h3 class="box-title m-b-0" title="{{ $value->nombre }}">
                          <a href="{{{ url('admin/medicos/view/' . $value->id) }}}">{{ substr($value->nombre,0,20) }}
                          </a>
                        </h3> <small>{{ $value->especialidad }}</small>
                        <p> </p>
                        <address>
                          <abbr title="Phone">T:</abbr> {{ $value->celular }}
                        </address>
                        <p></p>
                    </div>
                    ...
                    <div class="col-md-12">
                      ...
                      <div class="pull-right">
                        ...
                        <a title="Actualizar Informacion" href="{{ url('admin/medicos/edit/' . $value->id)}}" class="btn btn-twitter waves-effect waves-light">
                          <li class="fa fa-edit fa-lg"></li>
                        </a>
                        <?php } ?>
                        <?php if(Auth::user()->permisos->deleteRecord == 1) { ?>
                          <button type="button" data-toggle="tooltip" class="btn btn-danger waves-effect waves-light delete" data-url="<?php echo url("/"); ?>/admin/medicos/baja/<?php echo $value->id; ?>" data-title="Eliminar pacientes">
