    ...
    <div class="white-box">
      ...
      <div class="pull-left">
        <?php if(Auth::user()->permisos->addRecord == 1) { ?>
          <a href="{{ url('/admin/farmacia/add') }}" class="btn btn-info ">
      ...
      <div class="pull-right">
        <a href="{{ url('/admin/farmacia/excel' . $query) }}" class="btn btn-success" title="Exportar a Excel">
          <i class="fa fa-copy fa-2x"></i><br/>E. Excel
        </a>

        <a href="{{ url('/admin/farmacia/pdf' . $query) }}" class="btn btn-danger" title="Exportar a PDF">
              ...
              <table class="table display" id="table-content">
                ...
                <?php foreach($data as $value) { ?>
                  <tr id="hide<?php $value->id; ?>" >
				            <td> {{{ $value->cuarto->numero }}} {{{ $value->cuarto->descripcion }}} </td>
				            <td> {{{ $value->solicitante }}} </td>
                    <td> {{{ $value->fecha_registro }}} </td>
                    <td> {{{ $value->fecha_surtido }}} </td>
                    <td><button onclick="imprimeOrden({{{ $value->id }}});" class="<?php  echo $value->status == 1 ? 'btn btn-info btn-rounded' : 'btn btn-success btn-rounded'; ?>"> <?php  echo $value->status == 1 ? 'ACTIVA' : 'SURTIDA'; ?></button> </td>
                    <td>
                      <?php if(Auth::user()->permisos->addRecord == 1) { ?>
...
function imprimeOrden(id) {
  ...
  var url = "{{ url('admin/farmacia/view') }}/" + id;
