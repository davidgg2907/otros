...
class Pacientes extends Model
{
    ...
    public function getPacientesData($per_page, $request, $sortBy, $order){
      ...
      $pacientes->leftJoin('pacientes_asignacion','pacientes_asignacion.paciente_id','pacientes.id');
        ...
        $pacientes->distinct('pacientes_asignacion.paciente_id');
    ...
    public function updatePacientes($request){
      ...
      if(count($pacientes)){
          ...
          $pacientes->tratamiento = $request->input('tratamiento')!="" ? $request->input('tratamiento') : "";
    ...
    public function addPacientes($request){
      $pacientes = new Pacientes;
        ...
        $pacientes->nombre = $request->input('nombre')!="" ? $request->input('nombre') : "";
      	$pacientes->propietario_id = Auth::user()->medico_id;
        $pacientes->telefono = $request->input('telefono')!="" ? $request->input('telefono') : "";
      	$pacientes->celular = $request->input('celular')!="" ? $request->input('celular') : "";
      	$pacientes->domicilio = $request->input('domicilio')!="" ? $request->input('domicilio') : "";
      	$pacientes->tsangre = $request->input('tsangre')!="" ? $request->input('tsangre') : "";
        ...
        $pacientes->fc = $request->input('fc')!="" ? $request->input('fc') : 0;
        $pacientes->fr = $request->input('fr')!="" ? $request->input('fr') : 0;
        $pacientes->temperatura = $request->input('temperatura')!="" ? $request->input('temperatura') : 0;
        $pacientes->peso = $request->input('peso')!="" ? $request->input('peso') : 0;
        $pacientes->talla = $request->input('talla')!="" ? $request->input('talla') : 0;
        $pacientes->ta1 = $request->input('t1')!="" ? $request->input('t1') : 0;
        $pacientes->ta2 = $request->input('t2')!="" ? $request->input('t2') : 0;
        $pacientes->sato2 = $request->input('sato2')!="" ? $request->input('sato2') : 0;
    ...
    public function misAnalisis() {
      ...
      return $this->hasMany('\App\admin\Laboratorio', 'paciente_id', 'id')->orderBy('id','DESC');
