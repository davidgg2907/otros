...
class PacientesController extends Controller
{
    ...
    public function getSignosVitales($id){
      ...
      $pdf = PDF::loadView('admin/pacientes/signos', ['data'=>$data, 'config'=>$config,'empresa' => $empresa],
                                                [ 'title' => 'Expediente #' . $data->id, 'margin_top' => 0]);

  		return $pdf->stream('signos' . $data->id . '.pdf');
    ...
    public function getLaboratorioFicha($id) {

      $empresa   = \App\admin\Empresas::find(1);

      $data = \App\admin\Laboratorio::find($id);

      $pdf = PDF::loadView('admin/pacientes/analisis', ['data'=>$data, 'config'=>$config,'empresa' => $empresa],
                                                [ 'title' => 'Expediente #' . $data->id, 'margin_top' => 0]);

      return $pdf->stream('expediente' . $data->id . '.pdf');


      //return view('admin/pacientes/analisis', ['data'=>$data, 'config'=>$config,'empresa' => $empresa]);
